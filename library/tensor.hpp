 ...
 * @tparam T: data type
 * @tparam NDIMS: Tensor dimenstion
 */
...
struct tensor_view {
 ...
 public:
  ...
  template<int D = NDIMS>
  typename std::enable_if<(0 < D),
                          tensor_view<T, NDIMS - 1>>::type at(int idx) const {
...
struct tensor {
 ...
 public:
  ...
  template<int D = NDIMS>
  typename std::enable_if<(0 < D),
                          tensor_view<T, NDIMS - 1>>::type at(int idx) {
    return view().at(idx);
  }
  ...
  typename std::enable_if<(0 < D),
                          tensor_view<const T,
                                      NDIMS
                                          - 1>>::type operator[](int idx) const {
    return view()[idx];
  }
  template<int D = NDIMS>
  typename std::enable_if<(0 < D),
                          tensor_view<const T,
                                      NDIMS - 1>>::type at(int idx) const {
    return view().at(idx);
  }
