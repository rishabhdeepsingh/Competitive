...
template <size_t DIM>
AABB<DIM>::AABB(typename std::vector<UPoint<DIM>>::const_iterator iter,
                typename std::vector<UPoint<DIM>>::const_iterator end) {
...
private:
  ...
  void find_component_neighbors(size_t q, size_t r, size_t depth = 0) {
    if (is_fully_connected[q] && is_fully_connected[r] &&
        dsu.is_same(tree.points_begin(q)->get_id(), tree.points_begin(r)->get_id())) {
      return;
    }
    ...
    if (tree.is_leaf(q) && tree.is_leaf(r)) {
      ...
      for (auto i = tree.points_begin(q); i != tree.points_end(q); i++) {
        for (auto j = tree.points_begin(r); j != tree.points_end(r); j++) {
          if (!dsu.is_same(i->get_id(), j->get_id())) {
            double dist = distance(*i, *j);
            if (dist < nearest_set[dsu.get(i->get_id())].first) {
              nearest_set[dsu.get(i->get_id())] = {dist, {i->get_id(), j->get_id()}};
            }
          }
        }
        node_approximation[q] = std::max(node_approximation[q], nearest_set[dsu.get(i->get_id())].first);
      }
    } else {
  ...
  void check_fully_connected(size_t node_id) {
    ...
    if (tree.is_leaf(node_id)) {
      ...
      for (auto iter = tree.points_begin(node_id); iter + 1 != tree.points_end(node_id); ++iter) {
        fully_connected &= dsu.is_same(iter->get_id(), (iter + 1)->get_id());
      }
    ...
    if (is_fully_connected[left] && is_fully_connected[right] &&
        dsu.is_same(tree.points_begin(left)->get_id(), tree.points_begin(right)->get_id())) {
